...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        if (adb shell getprop ro.mithra.device | grep -q "$CM_BUILD"); then
...
function installboot()
{
    ...
    if (adb shell getprop ro.mithra.device | grep -q "$CM_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.mithra.device | grep -q "$CM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
